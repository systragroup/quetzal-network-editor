          ...
          params: [
            {
              ...
              text: $gettext('Max time between tasks'),
              type: 'Number',
              value: 0,
              units: 'mins',
              hint: $gettext('max waiting time before going back to the depot'),
              rules: ['required', 'largerThanZero'],
            },
            {
              cat: 'config',
              name: 'min_time_between_tasks',
              text: $gettext('Min time between tasks'),
              type: 'Number',
              value: 0,
              units: 'mins',
              hint: $gettext('minimum waiting time before each service, turnover time.'),
              rules: ['required', 'nonNegative'],
            },
            {
              cat: 'config',
              name: 'consumption',
              text: $gettext('bus consumption'),
              type: 'Number',
              value: 0,
              units: 'kwh/km',
              hint: $gettext('consumption.'),
              rules: ['required', 'nonNegative'],
            },
          ],
        },
          ...
          params: [
            {
              cat: 'config',
              name: 'max_energy',
              text: $gettext('effective Battery capacity'),
              type: 'Number',
              value: 0,
              units: 'KWH',
              hint: $gettext('battery capacity (including derating and operation margins)'),
              rules: ['required', 'largerThanZero'],
            },
            {
              cat: 'config',
              name: 'bus_cost',
              text: $gettext('bus Cost'),
              type: 'Number',
              value: 0,
              units: '$/Bus',
              hint: $gettext('bus cost for optimization'),
              rules: [],
            },
            {
              cat: 'config',
