...
// const endTime = ref('12:00:00')
const headway = ref(600)

const inputHeadway = ref(600)
...
watch(showSchedule, (val) => {
  if (val) { store.changeNotification({ text: '', autoClose: true })
    ...
    headway.value = links.value.features[0].properties.headway / 60
    inputHeadway.value = headway.value
    ...
    if (!isScheduleTrip(links.value.features[0])) {
      ...
      for (let step = 0; step < 20; step++) {
        let currentTime = hhmmssToSeconds(startTime.value)
        currentTime += headway.value
        startTime.value = secondsTohhmmss(currentTime)
...
function validate() {
  console.log(headway.value)
  const diff = (inputHeadway.value - headway.value) * 60
  headway.value = inputHeadway.value
  console.log(links.value)
  links.value.features.forEach(f => {
    f.properties.departures = f.properties.departures.map((el: string, i: number) => secondsTohhmmss(hhmmssToSeconds(el) + diff * i))
    f.properties.arrivals = f.properties.arrivals.map((el: string, i: number) => secondsTohhmmss(hhmmssToSeconds(el) + diff * i))
  })
  buildChartDataset()
}
